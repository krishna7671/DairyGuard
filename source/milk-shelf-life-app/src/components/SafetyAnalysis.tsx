...
export const SafetyAnalysis: React.FC<SafetyAnalysisProps> = ({ data }) => {
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">AI Confidence Score: <span className="font-bold">{analysis.score}%</span></p>
                        </div>
                    </div>
                    ...
                    <div className="w-16 h-16 rounded-full border-4 border-gray-200 flex items-center justify-center relative">
                        <div className={`absolute inset-0 rounded-full border-4 border-current opacity-20`} style={{ color: analysis.status === 'Safe' ? '#16a34a' : analysis.status === 'Caution' ? '#ca8a04' : '#dc2626' }}></div>
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                    <FileCheck className="w-5 h-5 text-purple-600" /> Indian Food Safety Compliance
                </h3>
                <div className="grid md:grid-cols-3 gap-4">

                    {/* Temp Compliance */}
                    <div className={`p-4 rounded-lg border flex flex-col items-center text-center ${data.temperature <= FSSAI_STANDARDS.temperature.max ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                        }`}>
                        ...
                        <span className="text-lg font-bold my-1">{data.temperature}°C</span>
                        <span className={`text-xs px-2 py-1 rounded-full ${data.temperature <= FSSAI_STANDARDS.temperature.max ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
                            }`}>
                            {data.temperature <= FSSAI_STANDARDS.temperature.max ? 'Compliant' : 'Non-Compliant'}
                        </span>
                        <span className="text-xs text-gray-400 mt-1">Standard: {FSSAI_STANDARDS.temperature.label}</span>
                    </div>

                    {/* pH Compliance */}
                    <div className={`p-4 rounded-lg border flex flex-col items-center text-center ${data.ph >= FSSAI_STANDARDS.ph.min && data.ph <= FSSAI_STANDARDS.ph.max ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                        }`}>
                        <Droplet className="w-6 h-6 mb-2 text-gray-700" />
                        <span className="text-xs font-semibold text-gray-500 uppercase">pH Level</span>
                        <span className="text-lg font-bold my-1">{data.ph}</span>
                        <span className={`text-xs px-2 py-1 rounded-full ${data.ph >= FSSAI_STANDARDS.ph.min && data.ph <= FSSAI_STANDARDS.ph.max ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
                            }`}>
                            {data.ph >= FSSAI_STANDARDS.ph.min && data.ph <= FSSAI_STANDARDS.ph.max ? 'Compliant' : 'Issue Detected'}
                        </span>
                        <span className="text-xs text-gray-400 mt-1">Standard: {FSSAI_STANDARDS.ph.label}</span>
                    </div>

                    {/* Bacteria Compliance */}
                    <div className={`p-4 rounded-lg border flex flex-col items-center text-center ${data.bacteriaCount <= FSSAI_STANDARDS.bacteria.max ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'
                        }`}>
                        ...
                        <span className="text-lg font-bold my-1">{(data.bacteriaCount / 1000).toFixed(1)}k</span>
                        <span className={`text-xs px-2 py-1 rounded-full ${data.bacteriaCount <= FSSAI_STANDARDS.bacteria.max ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
                            }`}>
                            {data.bacteriaCount <= FSSAI_STANDARDS.bacteria.max ? 'Compliant' : 'High Load'}
                        </span>
